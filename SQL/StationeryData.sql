...
-- Total Stationery Sold

SELECT SUM(quantity) AS Total_Stationery_Sold 
FROM orders_data;

-- Total Stationery Orders

SELECT COUNT(DISTINCT order_id) AS Total_Stationery_Orders 
FROM orders_data;

-- Average Stationery Per Order

SELECT CAST(CAST(SUM(quantity) AS DECIMAL(10,2)) / 
CAST(COUNT(DISTINCT order_id) AS DECIMAL(10,2)) AS DECIMAL(10,2))
AS Avg_Stationery_Per_Order
FROM orders_data;

-- % of Sales by Stationery Category

SELECT category, 
	CAST(SUM(sales) AS DECIMAL(10,2)) as Total_Sales,
		CAST(SUM(sales) * 100 / (SELECT SUM(sales) FROM orders_data) AS DECIMAL(10,2)) AS SCT
FROM orders_data
GROUP BY category;

-- % of Sales by Product ID
...
-- Top 5 Stationery Products Sold by Quantity
...
-- Bottom 5 Stationery Products Sold by Quantity
...
-- Total Stationery Sold By Category
...
-- Bottom 5 Customer Names By Sales
...
-- Top 5 Customer Names By Total Orders
...
-- Bottom 5 Customer Names By Total Orders
